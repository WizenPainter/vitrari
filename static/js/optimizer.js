...
class GlassOptimizer {
    ...
    async optimize(designs, sheet, algorithm = 'blf', options = {}) {
        ...
        try {
            ...
            const result = await this.algorithms[algorithm].optimize(pieces, sheet, opts);
        ...
        designs.forEach(design => {
            for (let i = 0; i < design.quantity; i++) {
                pieces.push({
                    ...
                    area: (design.design?.width || design.width) * (design.design?.height || design.height),
    ...
    calculateStatistics(result, sheet, pieces) {
        ...
        const usedArea = result.placedPieces.reduce((sum, piece) => sum + piece.area, 0);
        ...
        return {
            ...
            materialEfficiency: totalPieceArea > 0 ? (usedArea / totalPieceArea) * 100 : 0,
        ...
        for (let x = 0; x < sheet.width; x += resolution) {
            for (let y = 0; y < sheet.height; y += resolution) {
                if (!this.isPositionOccupied(x, y, occupiedAreas)) {
                    const wasteRect = this.findMaxEmptyRectangle(x, y, sheet, occupiedAreas, resolution);
                    maxWasteArea = Math.max(maxWasteArea, wasteRect.width * wasteRect.height);
                }
            }
        }
    ...
    compareOptimizations(optimizations) {
        ...
        let bestUtil = -1, bestEff = -1, fastestTime = Infinity;
...
`;
        ...
        optimization.result.placedPieces.forEach((piece, index) => {
            ...
            const dims = `${Math.round(piece.width)}×${Math.round(piece.height)}×${Math.round(piece.thickness || 6)}`.padEnd(16, ' ');
            const pos = `(${Math.round(piece.x)},${Math.round(piece.y)})`.padEnd(16, ' ');
...
- Total Cutting Length: ${optimization.cutPaths?.reduce((sum, path) => {
    ...
    async optimize(pieces, sheet, options) {
        ...
        for (const piece of sortedPieces) {
            ...
            if (placement) {
                ...
                this.updateAvailableSpaces(availableSpaces, placement, options.minimumGap);
            }
        }
